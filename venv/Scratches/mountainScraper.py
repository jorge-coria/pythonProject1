...
def get_mountains_urls(urls_filename='mountains_urls.pickle',
                       url='https://www.mountain-forecast.com/countries/United-States?top100=yes'):
    ...
    try:
        ...
        mountain_urls = {item.find('a').get_text(): get_urls_by_elevation(item.find('a')['href']) for item in
                         mountain_items}
...
def save_data(rows):
    ...
    dataset_name = os.path.join(os.getcwd(), '{:02d}{}_mountain_forecasts.csv'.format(today.month,
                                                                                      today.year))  # i.e. 042019_mountain_forecasts.csv
    ...
    for mountain_name, urls in mountains_urls.items():
        ...
        for url in urls:
            ...
            forecast_table = soup.find('table',
                                       attrs={'class': 'forecast__table forecast__table--js'})  # Default unit is metric
            ...
            for i, day in enumerate(days):
                ...
                if current_day != '':  # What if day is empty in the middle? Does it affect the count?
                    ...
                    date = str(datetime.date(datetime.date.today().year, datetime.date.today().month, int(
                        current_day.split(' ')[
                            1])))  # Avoid using date format. Pandas adds 00:00:00 for some reason. Figure out better way to format
